...
namespace Cooperchip.ITDeveloper.Domain.Mensageria.Validations
{
    public class TriagemValidation : AbstractValidator<Triagem>
    {
        public TriagemValidation()
        {
            ...
            RuleFor(x => x.CodigoPaciente).NotNull().WithMessage("O campo '{PropertyName}' é obrigatório");
            RuleFor(x => x.Mensagem).NotNull().WithMessage("O campo '{PropertyName}' é obrigatório")
